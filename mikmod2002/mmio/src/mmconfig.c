...
// will change the sub-section - no other red-tape involved.
{
    ...
    if(conf->flags & MMCONF_CASE_INSENSITIVE) _strlwr(work);
        ...
        if(!conf->lineflg[lpos] && (line = conf->line[lpos]))
        {
            ...
            {   case  '[':  return -1;
                ...
                default:
                    ...
                    if(conf->flags & MMCONF_CASE_INSENSITIVE) _strlwr(conf->work);
...
// Returns the number of values actually found and loaded (never greater than 'nent')
{
    ...
    {   uint   t;
        ...
        {   int   i=0;
            ...
            if(conf->flags & MMCONF_CASE_INSENSITIVE) _strlwr(conf->work2[t]);
